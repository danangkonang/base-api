...
import (
	...
	"github.com/danangkonang/crud-rest/db/migration"
	"github.com/danangkonang/crud-rest/db/seeder"
...
func migrationDown(t *Tables) {
	...
	for _, migrate := range t.NameTable {
		...
		fmt.Println(string(green), "success", string(reset), "DROP ", migrate)
	}
}
...
func seederDown(t *Tables) {
	...
	for _, migrate := range t.NameTable {
		...
		fmt.Println(string(green), "success", string(reset), "remove data on TABLE ", migrate)
	}
}
...
func migrationUp(t *Tables) {
	...
	for _, migrate := range t.NameTable {
		list := strings.Split(migrate, "_migration_")
		tb_name := strings.Split(list[1], ".go")
		meth := reflect.ValueOf(&m).MethodByName(strings.Title(tb_name[0]))
		meth.Call(nil)
	}
}
